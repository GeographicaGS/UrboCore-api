node("docker") {
    ...
    try {
        ...
        stage "Building"
            ...
            echo "Building urbo-api/${build_name}"

            sh "cp test/config.test.yml config.yml"
            sh "docker build --pull=true -t geographica/urbo_api -f Dockerfile.test ."
        ...
        stage "Testing"
            ...
            echo "Linter test"
            sh "docker run -i --rm --name urbo_api--${build_name} -e 'NODE_ENV=development' geographica/urbo_api npm run lint"
        ...
        echo "urbo-api/${build_name} failed: ${error}"
        throw error
        ...
        stage "Cleaning"
            ...
            echo "Cleaning urbo-api/${build_name}"
        ...
        if (currentBuild.result == "SUCCESS" && ["master", "dev"].contains(branch_name)) {
            ...
            stage "Deploying"
                ...
                if (branch_name == "master") {
                    ...
                    sh "ansible urbo-production -a 'sh /data/app/urbo/urbo-api/deploy.sh'"

                } else if (branch_name == "dev") {
                    echo "Deploying dev ..."
                    sh "ansible urbo-dev -a 'sh /data/app/urbo/urbo-api/deploy.sh'"
